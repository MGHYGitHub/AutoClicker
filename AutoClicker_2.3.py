...
class AutoClickerApp:
    def __init__(self, root):
        ...
        # Program state
        # click_points: list of dicts: {'x':int,'y':int,'name':str,'button':'left'/'right','delay':float}
        self.click_points = []
    ...
    # ---------------- Points management ----------------
    def add_point(self, x, y, name=None, button=None, delay=None):
        if name is None:
        ...
        p = {'x': int(x), 'y': int(y), 'name': str(name), 'button': button, 'delay': float(delay)}
        self.click_points.append(p)
        self.update_points_list()
        self.add_progress_text(f"添加点位: {name} ({x},{y}) 键:{button} 延时:{delay}")
        log(f"添加点位 {name} ({x},{y})")

    def add_current_point(self):
        try:
            x, y = pyautogui.position()
            # pop-up to choose button & delay
            dlg = AddPointDialog(self.root, x, y, default_button=self.default_button_var.get(), default_delay=self.base_delay_var.get())
            self.root.wait_window(dlg.top)
            if dlg.result:
                name, btn, delay = dlg.result
    ...
    def update_points_list(self):
        ...
        for i, p in enumerate(self.click_points):
            display = f"{i+1}. {p['name']} - ({p['x']},{p['y']}) [{p['button']}] delay:{p['delay']}"
    ...
    def edit_selected_point(self, event=None):
        ...
        idx = sel[0]
        p = self.click_points[idx]
        dlg = EditPointDialog(self.root, p['x'], p['y'], p['name'], p['button'], p['delay'])
        self.root.wait_window(dlg.top)
        if dlg.result:
            nx, ny, nname, nbutton, ndelay = dlg.result
            self.click_points[idx] = {'x': int(nx), 'y': int(ny), 'name': nname, 'button': nbutton, 'delay': float(ndelay)}
            self.update_points_list()
            self.add_progress_text(f"点位已编辑: {nname} ({nx},{ny}) 键:{nbutton} 延时:{ndelay}")
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    if isinstance(item, list) and len(item) >= 3:
                        ...
                        new_list.append({'x':x,'y':y,'name':name,'button':self.default_button_var.get(),'delay':self.base_delay_var.get()})
                        ...
                        new_list.append({'x':x,'y':y,'name':name,'button':button,'delay':delay})
    ...
    def start_task(self):
        ...
        # validate params
        click_count = safe_int(self.click_count_var.get(), 1)
    ...
    # 修改这里：根据设置决定是否显示确认窗口
        ...
        # run task thread
        self.task_thread = threading.Thread(target=self.run_click_task, args=(click_count, base_delay, loop_count, offset_px, rand_delay), daemon=True)
    ...
    def run_click_task(self, click_count, base_delay, loop_count, offset_px, rand_delay):
        total_points = len(self.click_points)
        total_ops = total_points * click_count * loop_count
        ops_done = 0
        ...
        try:
            for loop in range(loop_count):
                ...
                for idx, p in enumerate(self.click_points):
                    ...
                    cur_info = f"当前: 循环 {loop+1}/{loop_count} — 点位 {idx+1}/{total_points} - {p['name']} ({p['x']},{p['y']}) 键:{p['button']}"
                    ...
                    for c in range(click_count):
                        ...
                        # delay inside clicks
                        delay = p.get('delay', base_delay)
                        if rand_delay > 0:
                            delay = delay + random.uniform(-rand_delay, rand_delay)
                            if delay < 0:
                                delay = 0
    ...
    def global_capture_coord(self):
        if self.is_capturing:
            try:
                x, y = pyautogui.position()
                # when capturing via global hotkey, use default button/delay
                self.add_point(x, y)
    ...
    def check_mouse_movement(self):
        try:
            ...
            # 使用UI中设置的动态阈值，而不是固定的 self.mouse_move_threshold_px
            ...
            # 2. 任务运行中且未手动暂停
            # 3. 移动距离超过阈值
            # 4. 距离上次触发至少10秒（避免频繁触发）
            if (self.enable_safety_check.get() and 
                self.is_running and 
                not self.pause_event.is_set() and 
                dist > threshold and
                current_time - self.last_safety_trigger > 10):
    ...
    def write_click_report(self, silent=False):
        try:
            ...
            content.append(f"参数: click_count={self.click_count_var.get()}, base_delay={self.base_delay_var.get()}, loop={self.loop_var.get()}, random_offset={self.random_offset_var.get()}, random_delay={self.random_delay_var.get()}")
            content.append("点位列表:")
            for i, p in enumerate(self.click_points, 1):
                content.append(f"  {i}. {p['name']} ({p['x']},{p['y']}) 按键:{p['button']} 延时:{p['delay']}")
            content.append("统计:")
...
class AddPointDialog:
    def __init__(self, parent, x, y, default_button='left', default_delay=1.0):
...
class EditPointDialog:
    def __init__(self, parent, x, y, name, button, delay):
        self.top = tk.Toplevel(parent)
        self.top.title("编辑点位")
        self.top.resizable(False, False)
    ...
    def on_ok(self):
        ...
        try:
            ...
            return
        self.result = (nx, ny, nname, nbutton, ndelay)
        self.top.destroy()
